// Lovable Cloud Edge Function: fetch-devpost-hackathons
// Runs the Apify Devpost scraper actor at most once every 24h, dedupes results,
...
function mapItem(item: ApifyItem): HackathonInsert | null {
  ...
  const source_url = pickString(item, "url", "source_url", "link");
  if (!title || !source_url) return null;

  const tagsRaw = item.tags ?? item.themes ?? item.topics;
  ...
  return {
    ...
    mode: normalizeMode(pickString(item, "mode", "location_type", "format")),
    city: pickString(item, "city"),
    country: pickString(item, "country"),
    registration_deadline: toDate(item.registration_deadline ?? item.deadline ?? item.submissionDeadline),
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    console.error("fetch-devpost-hackathons error:", message);
